...
public:
	...
	DATA* Alloc(Args... args)
	{
		...
		if (pNodeBucket != nullptr && pNodeBucket->allocCount >= BUCKET_SIZE)
		{
			__debugbreak();
		}
		if (pNodeBucket == nullptr)
		{
			...
			if (pNodeBucket->allocCount < 0 || pNodeBucket->allocCount >= BUCKET_SIZE)
			{
				__debugbreak();
			}
		...
		InterlockedIncrement(&(pNodeBucket->allocCount));
		LONG localAllocCount = pNodeBucket->allocCount;
		Sleep(0);
		Sleep(0);
		if (localAllocCount != pNodeBucket->allocCount)
			__debugbreak();
