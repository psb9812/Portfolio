...
NetServer::NetServer()
	...
	_packetCode(0), _encodeStaticKey(0), _timeoutDisconnectForSession(0), _timeoutDisconnectForUser(0), _defaultContentID(std::nullopt)
{
...
void NetServer::HandleTimerPQCS()
{
	OnTime();
}

void NetServer::HandleMoveContentsPQCS(DWORD transferred, INT64 sessionID)
{
	Session* pSession = _sessionContainer.SearchSession(sessionID);
	if (pSession == nullptr)
	{
...
void NetServer::HandleFrameLogicPQCS(DWORD transferred, INT64 sessionID)
{
...
bool NetServer::SendPacket(UINT64 sessionID, Message* pMessage, bool usePQCS, bool disconnect)
{
	pMessage->AddRefCount();
...
void NetServer::RequestMoveContents(UINT64 sessionID, INT32 destContentsID)
{
...
HANDLE NetServer::TimerThreadStarter()
{
	auto threadFunc = [](void* pThis) -> unsigned int
		{
			NetServer* pNetServer = static_cast<NetServer*>(pThis);
			pNetServer->TimerThread();
			return 0;
		};
