...
bool Session::RequestRecv()
{
	...
	//WSARecv를 1번만 요청하기 위한 목적
	if (InterlockedExchange(&_isRecving, TRUE) == TRUE)
		return false;
...
int Session::RequestSend()
{
	...
	for (int i = 0; i < MAX_SEND_BUF_ONCE; i++)
	{
...
bool Session::LeaveContents()
{
	if (_contentsID.has_value())
	{
		_contentsID.reset();
		return true;
	}
