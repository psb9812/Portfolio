...
{
	_sessionArray = new Session[numOfMaxUser];

	for (int i = 0; i < numOfMaxUser; i++)
	{
...
{
	//sessionID로 인덱스 찾기
	USHORT targetIndex = static_cast<USHORT>(sessionID >> ID_BITWIDE);

	if (_sessionArray[targetIndex].IsEmptyIndex())
	{
		return nullptr;
	}
	else
...
void SessionContainer::DeleteSession(__int64 sessionID)
{
	...
	_sessionArray[targetIndex].ReleaseSession();
...
Session* SessionContainer::InsertSession(__int64 ID, SOCKET clientSock, IN_ADDR clientIP, int clientPort)
{
	...
	if (_sessionArray[insertIndex].IsEmptyIndex() == false)
	{
		//꺼냈던 것 다시 넣기
