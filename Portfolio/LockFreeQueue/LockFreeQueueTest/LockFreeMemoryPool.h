...
public:
		...
		:_capacity(blockNum), _useCount(0), _usePlacementNew(placementNew), _pTopNode(NULL), _addrIDCount(0)
	{
		...
		for (int i = 0; i < _capacity; i++)
		{
			...
			if (prevNode == nullptr)
			{
				...
				UINT_PTR curNodeMetaAddr = AddressConverter::InsertMetaIDToAddress(reinterpret_cast<UINT_PTR>(curNode), addrID);
	...
	~LockFreeMemoryPool()
	{
		Node* pRealTopNode = reinterpret_cast<Node*>(AddressConverter::DeleteMetaIDToAddress(_pTopNode));
		//현재 내가 들고 있는 메모리만 해제해준다.
		while (pRealTopNode != nullptr)
		{
			Node* delNode = pRealTopNode;
			pRealTopNode = reinterpret_cast<Node*>(AddressConverter::DeleteMetaIDToAddress((UINT_PTR)(pRealTopNode->next)));
			free(delNode);
		}
	}
	...
	bool Free(DATA* pData)
	{
		...
		while (true)
		{
			...
			//newNode의 주소를 가공한다.
			UINT64 addrID = AddressConverter::GetAddressID(&_addrIDCount);
			UINT_PTR newNodeMetaAddr = AddressConverter::InsertMetaIDToAddress((UINT_PTR)newNode, addrID);
