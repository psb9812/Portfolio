...
public:
		...
		:_capacity(bucketNum), _useCount(0), _usePlacementNew(placementNew), _pTopNodeBucket(NULL), _addrIDCount(0)
	{
		...
		for (int i = 0; i < _capacity; i++)
		{
			...
			if (prevNodeBucket == nullptr)
			{
				...
				UINT_PTR curNodeMetaAddr = AddressConverter::InsertAddressIDToAddress(reinterpret_cast<UINT_PTR>(curNodeBucket), addrID);
	...
	~TLSMemoryPool()
	{
		NodeBucket* pRealTopNode = reinterpret_cast<NodeBucket*>(AddressConverter::DeleteAddressIDFromAddress(_pTopNodeBucket));
		//현재 내가 들고 있는 메모리만 해제해준다.
		while (pRealTopNode != nullptr)
		{
			NodeBucket* delNode = pRealTopNode;
			pRealTopNode = reinterpret_cast<NodeBucket*>(AddressConverter::DeleteAddressIDFromAddress((UINT_PTR)(pRealTopNode->next)));
			delete delNode;
	...
	NodeBucket* AllocFromPool(Args... args)
	{
		...
		while (true)
		{
			...
			NodeBucket* pLocalTopRealAddr = reinterpret_cast<NodeBucket*>(AddressConverter::DeleteAddressIDFromAddress(pLocalTopMetaAddr));
	...
	bool FreeToPool(NodeBucket* pFreeBucket)
	{
		...
		while (true)
		{
			...
			UINT64 addrID = AddressConverter::GetAddressID(&_addrIDCount);
			UINT_PTR pFreeBucketMetaAddr = AddressConverter::InsertAddressIDToAddress((UINT_PTR)pFreeBucket, addrID);
